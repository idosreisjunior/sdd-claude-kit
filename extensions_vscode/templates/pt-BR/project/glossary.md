...
{{guia: a seção "Termos do método" vem pronta e raramente muda. A seção
"Termos do domínio" é onde está o valor real: é ela que impede que "cliente",
"conta" e "usuário" signifiquem três coisas diferentes em três specs.}}
...
| Termo | Definição |
| --- | --- |
| {{DOMAIN_TERM}} | {{DOMAIN_DEFINITION}} |

{{repetir: uma linha por termo do negócio}}

{{guia: registre principalmente os termos ambíguos — aqueles em que duas pessoas
da equipe dariam definições diferentes. Termos óbvios não precisam entrar.}}

## Termos evitados

| Não usar | Usar | Por quê |
| --- | --- | --- |
| {{AVOIDED_TERM}} | {{PREFERRED_TERM}} | {{REASON}} |

{{repetir: sinônimos que causam confusão. Remova a seção se não houver.}}
