...
| WIZ-009..015 | ⚪ pending | hub · IA · views · aprovar · implementar · verificar · a11y |

Fonte da verdade: `.specs/features/0035-wizard-cockpit/` (`status.yaml`, `tasks.md`,
`traceability.yaml`). Validação atual: `npm run compile` ✓ · `npm test` (203) ✓ · `npm run lint` ✓.
...
- **WIZ-010 ações de IA:** extrair `runHybridStep` de `extension.ts` para `hybridStep.ts` e
  disparar `/sdd-kit:<ação>` no terminal (sem enviar); fallback copiar prompt (SCN-WIZ-012).
- **WIZ-011 views de conteúdo:** Especificar/Clarificar/Desenhar/Tarefas exibindo dados de
  `dashboardModel`/`taskAnalysis`/`decisions`.
